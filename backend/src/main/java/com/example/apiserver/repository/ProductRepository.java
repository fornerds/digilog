...
public interface ProductRepository extends BaseRepository<Product, Long> {
    ...
    @Query("SELECT DISTINCT p FROM Product p " +
           ...
           "WHERE p.deletedAt IS NULL AND psc.deletedAt IS NULL " +
    ...
    @Query("SELECT DISTINCT p FROM Product p " +
           ...
           "WHERE p.deletedAt IS NULL AND ppc.deletedAt IS NULL " +
    ...
    @Query("SELECT p FROM Product p WHERE p.deletedAt IS NULL " +
           ...
           "AND (:skinCode IS NULL OR :skinCode = '' OR EXISTS (SELECT 1 FROM ProductSkinCode psc WHERE psc.product.id = p.id AND psc.skinCode = :skinCode AND psc.deletedAt IS NULL)) " +
           "AND (:personalColor IS NULL OR :personalColor = '' OR EXISTS (SELECT 1 FROM ProductPersonalColor ppc WHERE ppc.product.id = p.id AND ppc.personalColor = :personalColor AND ppc.deletedAt IS NULL)) " +
           "ORDER BY p.createdAt DESC")
